...
def rolling_volatility(returns, rolling_vol_window):
    ...
    return returns.rolling(rolling_vol_window).std() * np.sqrt(252)

def rolling_alpha(returns, factor_returns, rolling_window=APPROX_BDAYS_PER_MONTH * 6):
    """
    ...
    if factor_returns.ndim > 1:
        ...
        return factor_returns.apply(
            partial(rolling_alpha, returns), rolling_window=rolling_window
        )
    else:
        ...
        for beg, end in zip(
            ...
            out.loc[end] = ep.alpha(returns.loc[beg:end], factor_returns.loc[beg:end])
...
def rolling_var(returns, rolling_var_window):
    """
    Determines the rolling Sharpe ratio of a strategy.
    Parameters
    ----------
    ...
    return (
        returns.rolling(rolling_var_window).var())

def extract_interesting_date_ranges(returns, periods=PERIODS):
    """
